...
st.write("# Welcome to RAG-Evaluator! 👋")
...
options = ['cohere.embed-english-v3','cohere.embed-multilingual-v3','amazon.titan-embed-text-v1']
embedding = st.selectbox('1、Choose embedding Model:', options)
if st.button('save embedding model'):
# if embedding:
    tagging = {
                    'TagSet': [
                        {
    ...
    s3.put_bucket_tagging(Bucket=f'rag-input-{tenantid}',Tagging=tagging)
    st.markdown(f'embedding model is :blue[{embedding}]')
    
        # 读取对象内容
    


uploaded_rag_file = st.file_uploader("2、Choose A File Use For Rag",type=['pdf', 'txt', 'html', 'doc', 'docx', 'csv', 'xls', 'xlsx'])
if uploaded_rag_file is not None:
    ...
    if st.button("Upload RAG File"):
        ...
        with st.spinner('Building knowledge base'):
            ...
            try:
                iam.get_role(RoleName=bedrock_kb_execution_role)
                st.write('Role already exists, skipping creation')
                ...
                try:
                    ...
                    st.write('Role created successfully')
            ...
            try:
                ...
                print(f'Collection "{vector_store_name}" created successfully.')
            except Exception as e:
                # 处理其他异常
                st.write(f'Error creating collection: {e}')        
            ...
            response = s3.get_bucket_tagging(Bucket=f'rag-input-{tenantid}')
            tags = response['TagSet']
            for tag in tags:
                if tag['Key'] == 'embedding_model':
                    embed = tag['Value']
                    st.write(f"embed: {tag['Value']}")
                    break
            if embed != 'amazon.titan-embed-text-v1':
                dimension = 1024
            else:
                dimension = 1536
            #   Create vector index
            ...
            embeddingModelArn = f"arn:aws:bedrock:{region}::foundation-model/{embed}"
...
options = ['cohere.command-text-v14','cohere.command-light-text-v14','anthropic.claude-instant-v1','anthropic.claude-v2','anthropic.claude-3-sonnet-20240229-v1:0','anthropic.claude-3-haiku-20240307-v1:0','mistral.mistral-7b-instruct-v0:2','meta.llama2-13b-chat-v1','meta.llama2-70b-chat-v1','anthropic.claude-v2:1', 'ai21.j2-ultra-v1','ai21.j2-mid-v1','amazon.titan-text-lite-v1','amazon.titan-text-express-v1',]
selected = st.multiselect('3、Choose LLM Model:', options, default=options[0])
#打印出选择的大语言模型
if st.button('submit'):
    st.write('You have choosed llm:', selected)
...
uploaded_QA_file = st.file_uploader("4、Upload Questions and Answers File", type=["json"])
if uploaded_QA_file :
    ...
    if selected == []:
        ...
        if st.button("Upload Questions and Answers File"):
            ...
            s3.delete_object(Bucket=f'model-eva-output-{tenantid}',Key='answer-similarity/answer-similarity.json')
